...
public class CustomCalloutBalloonAdapter implements CalloutBalloonAdapter{
    ...
    private MapPOIItem createMarker(MapPOIItem poiItem) {
        int tag = poiItem.getTag();
        String infoString = info.get(tag);

        String[] infoArray = (infoString != null) ? infoString.split(",") : new String[0];

        double latitude = (infoArray.length > 1) ? Double.parseDouble(infoArray[1]) : 0.0;
        double longitude = (infoArray.length > 2) ? Double.parseDouble(infoArray[2]) : 0.0;

        String csName = (infoArray.length > 9) ? infoArray[9] : "";

        String markerName = csName + " " + tag + "번";
